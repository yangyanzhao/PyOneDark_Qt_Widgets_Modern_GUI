...
class View(object):
    ...
    def init_view(self):
        '''基本框架'''
        self.frm_main = ttk.LabelFrame(self.root, borderwidth=0)
        self.frm_main.pack(side='left', fill='y')

        self.frm_advance = ttk.LabelFrame(self.root, text='高级选项')
        # self.frm_advance.pack(expand='yes', side='right', fill='both', padx=15, pady=10)
        # self.frm_2 = ttk.LabelFrame(self.frm_advance, text='高级配置', width=300)
        # self.frm_2.pack(expand='yes', side='top', fill='both', padx=15, pady=10)

        self.frm_project = ttk.LabelFrame(self.frm_main, text='项目信息')
        self.frm_config = ttk.LabelFrame(self.frm_main, text='配置信息')
        self.frm_operate = ttk.LabelFrame(self.frm_main, text='操作')
        self.frm_status = ttk.LabelFrame(self.frm_main, text='状态')
    ...
    def init_project(self):
        ...
        for index, label_text in enumerate(labels):
            ...
            temp_label = ttk.Label(self.frm_project, text=label_text)
            temp_entry = ttk.Entry(self.frm_project,
        ...
        self.btn_main_path = ttk.Button(self.frm_project,
                                        text='选择文件',
                                        command=self.fn_select_main)
        self.btn_work_path = ttk.Button(self.frm_project,
                                        text='选择路径',
                                        command=self.fn_work_path)
        self.btn_dist_path = ttk.Button(self.frm_project,
                                        text='选择路径',
                                        command=self.fn_dist_path)
        self.btn_ico_path = ttk.Button(self.frm_project,
                                       text='选择图标',
    ...
    def init_config(self):
        ...
        # 子配置框架
        self.frm_config_base = ttk.LabelFrame(self.frm_config,
                                              text='基本',
                                              borderwidth=0)
        self.frm_config_base.pack(fill='x', padx=10, pady=5, ipady=5)
        self.frm_config_exe = ttk.LabelFrame(self.frm_config,
                                             text='生成执行文件类型',
                                             borderwidth=0)
        self.frm_config_exe.pack(fill='x', padx=10, pady=5, ipady=5)
        self.frm_config_other = ttk.LabelFrame(self.frm_config,
                                               text='其它',
                                               borderwidth=0)
        self.frm_config_other.pack(fill='x', padx=10, pady=5, ipady=5)
        self.frm_config_spec = ttk.LabelFrame(self.frm_config,
                                              text='配置文件',
                                              borderwidth=0)
        self.frm_config_spec.pack(fill='x', padx=10, pady=5, ipady=5)

        # 定义按钮
        self.btn_noconsole = ttk.Checkbutton(self.frm_config_base,
                                             text='关闭控制台',
                                             variable=self.cfg_noconsole)
        self.btn_clean = ttk.Checkbutton(self.frm_config_base,
                                         text='构建前清理',
                                         variable=self.cfg_clean)
        self.btn_upx = ttk.Checkbutton(self.frm_config_base,
                                       text='UPX压缩',
                                       variable=self.cfg_upx)
        self.btn_isonefile = ttk.Checkbutton(self.frm_config_exe,
                                             text='独立执行文件',
                                             variable=self.cfg_onefile)
        self.btn_isonedir = ttk.Checkbutton(self.frm_config_exe,
                                            text='文件夹包含',
                                            variable=self.cfg_onedir)
        self.btn_rename = ttk.Checkbutton(self.frm_config_other,
                                          text='修改执行文件名',
                                          variable=self.cfg_rename)
        self.entry_rename = ttk.Entry(self.frm_config_other,
                                      textvariable=self.cfg_exe_name)

        # self.btn_rename = ttk.Checkbutton(self.frm_config_spec, text='生成配置文件', variable=self.cfg_specfile)
        self.entry_specfile = ttk.Entry(self.frm_config_spec,
    ...
    def init_operate(self):
        '''操作命令'''
        # 定义按钮
        self.btn_build = ttk.Button(self.frm_operate,
                                    text='构建生成',
                                    command=self.fn_build)
        self.btn_clear = ttk.Button(self.frm_operate,
                                    text='清理',
                                    command=self.fn_clear)
        self.btn_reset = ttk.Button(self.frm_operate,
                                    text='重置',
                                    command=self.fn_reset)
        self.btn_advance = ttk.Button(self.frm_operate,
                                      text='高级选项',
    ...
    def init_status(self):
        '''状态栏'''
        self.label_status = ttk.Label(self.frm_status, text='待命')
