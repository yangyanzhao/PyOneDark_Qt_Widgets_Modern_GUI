...
class BackgroundWrapper(QWidget):
    def __init__(self, source_widget: QWidget, background_image: str):
        super(BackgroundWrapper, self).__init__()
...
if __name__ == '__main__':
    ...
    # 背景图
    background_wrapper = BackgroundWrapper(source_widget=login_window, background_image=icons[f'background_0.jpg'])
    # 边框窗口
    # frameless_window_wrapper = FramelessWindowWrapper(target_widget=background_wrapper, has_title_bar=True,attach_title_bar_layout=background_wrapper.source_widget.verticalLayout_1)
    # login_window.set_wrapper(frameless_window_wrapper)

    # MTheme("dark").apply(frameless_window_wrapper)
    background_wrapper.show()
    with loop:
        loop.run_forever()
