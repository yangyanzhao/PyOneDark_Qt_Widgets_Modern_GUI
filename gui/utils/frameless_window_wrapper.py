...
class FramelessWindowWrapper(QWidget):
    ...
    def update_background(self):
        # 设置背景图片的路径
        background_image = icons['background_0.jpg']
        # 创建一个 QPixmap 对象
        pixmap = QPixmap(background_image)
        # 调整图片大小以适应窗口大小
        scaled_pixmap = pixmap.scaled(self.center_widget.size(), Qt.KeepAspectRatioByExpanding,
                                      Qt.SmoothTransformation)
        # 创建一个 QPalette 对象
        palette = self.center_widget.palette()

        # 设置 QPalette 的背景图片
        palette.setBrush(QPalette.Window, scaled_pixmap)

        # 应用 QPalette 到主控件
        self.center_widget.setPalette(palette)
        # 设置自动填充背景
        self.center_widget.setAutoFillBackground(True)
...
if __name__ == '__main__':
    ...
    demo_window_wrapper = FramelessWindowWrapper(target_widget=demo_window, has_title_bar=True,
                                                 attach_title_bar_layout=demo_window.v_layout)
    # 显示窗口
